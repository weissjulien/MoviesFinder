...
try:
    ...
    st.sidebar.success("✅ Loaded TMDB key from secrets")
except:
...
if not tmdb_api_key:
    ...
    **Option 1 (Recommended - No pasting needed):**
    1. Create file: `.streamlit/secrets.toml`
    2. Add: `TMDB_API_KEY = "your_api_key_here"`
    3. Refresh browser - key loads automatically!
    
    **Option 2 (One-time paste):**
    Just paste your key in the sidebar above.
    
    **Get your free API key:** https://www.themoviedb.org/settings/api
    """)
    st.stop()

# Try to load OMDB key from secrets
omdb_api_key = None
try:
...
if not genres_dict:
    st.error("❌ Failed to connect to TMDB. Check your API key at: https://www.themoviedb.org/settings/api")
    st.stop()
...
with st.spinner("🔄 Loading popular movies..."):
    ...
    if show_debug:
        st.info(f"📊 Loaded {len(popular_movies)} popular movies from TMDB")

st.markdown("---")
with st.spinner("🔍 Checking streaming availability in Germany..."):
    netflix_movies = []
    prime_movies = []
    all_with_streaming = []
    movies_without_streaming = 0
    ...
    for movie in popular_movies:
        ...
        # Track movies with streaming availability
        if watch_providers:
            all_with_streaming.append(movie_data)
...
with col1:
    st.metric("Netflix (DE)", len(netflix_movies))
with col2:
    st.metric("Prime Video (DE)", len(prime_movies))
with col3:
    st.metric("Total Popular", len(popular_movies))

if show_debug:
    st.info(f"""
    **Debug Info:**
    - Movies with streaming data: {len(all_with_streaming)}
    - Movies without streaming data: {movies_without_streaming}
    - This means TMDB watch/providers endpoint only has data for {len(all_with_streaming)}/{len(popular_movies)} popular movies
    - If only a few movies found, TMDB's Germany watch/providers data may be limited
    """)

if not all_with_streaming:
    st.warning("⚠️ No streaming data found in TMDB for these movies.")
    st.info("""
    This might mean:
    - TMDB watch/providers data is limited for your region
    - Your API key might not have watch/providers permission
    - Try getting a fresh API key at: https://www.themoviedb.org/settings/api
    """)
    st.stop()
...
st.divider()
st.markdown("**MoviesFinder v4.2** | Streaming Data: TMDB | Germany (DE) | 📱 Streamlit")
